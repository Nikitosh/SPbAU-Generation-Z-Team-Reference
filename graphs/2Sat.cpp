...
inline void Implies(int v1, int v2) { g[v1].pb(v2), rg[v2].pb(v1); }

inline void Or(int v1, int v2) { Implies(Not(v1), v2), Implies(Not(v2), v1); }
...
void dfs1(int v) {
  usedSat[v] = 1;
  for (int to : g[v]) if (!usedSat[to]) dfs1(to);
  topsortSat.pb(v);
}
 
void dfs2(int v, int c) {
  comp[v] = c;
  for (int to : rg[v]) if (!comp[to]) dfs2(to, c);
}
