...
void fft(Num *a, int n) {
  ...
  int q = N / n;
  forn (i, n) {
    if (i < rev[i] / q) swap(a[i], a[rev[i] / q]);
  }
  for (int k = 1; k < n; k <<= 1) {
...
void doubleFft(Num *a, Num *fa, Num *fb, int n) { // only if you need it
  fft(a, n);
  const int n1 = n - 1;
  forn (i, n) {
...
void mult(T *a, T *b, T *r, int n) { // n = 2^k
  forn (i, n) tmp[i] = Num((dbl) a[i], (dbl) b[i]);
  fft(tmp, n);
  const int n1 = n - 1;
  ...
  fft(tmp, n);
  forn (i, n) r[i] = (T) round(tmp[i].real());
}                  

void init() { // don't forget to init
  forn(i, N) rev[i] = (rev[i >> 1] >> 1) + ((i & 1) << (LOG - 1));
  rt[1] = Num(1, 0);
  for (int k = 1, p = 2; k < LOG; k++, p *= 2) {
    const Num x(cos(PI / p), sin(PI / p));
    forab (i, p / 2, p) rt[2 * i] = rt[i], rt[2 * i + 1] = rt[i] * x;
  }
}
