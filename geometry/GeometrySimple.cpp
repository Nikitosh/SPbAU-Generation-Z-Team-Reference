...
inline bool subIntr(const Pnt &a, const Pnt &b, const Pnt &c, const Pnt &d){
  return sign((b - a) % (c - a)) * sign((b - a) % (d - a)) == -1 &&
...
int containsSlow(Pnt p, Pnt *z, int n){
  ...
  forn (j, n){
    Pnt a = z[j], b = z[(j + 1) % n];
    if (inSeg(p, a, b)) return -1; // border
...
bool containsFast(Pnt p, Pnt *z, int n) {
  Pnt o = z[0];
  if (gr((p - o) % (z[1] - o), 0) || ls((p - o) % (z[n - 1] - o), 0)) return 0;
  int l = 0, r = n - 1;
  while(r - l > 1){
    int m = (l + r) / 2;
    if (gr((p - o) % (z[m] - o), 0)) r = m;
    else l = m;
  }
...
inline int isInTr(const Pnt &p, const Pnt &a, const Pnt &b, const Pnt &c){
  return gr((b - a) % (p - a), 0) && gr((c - b) % (p - b), 0) && gr((a - c) % (p - c), 0);
}
